...
private:
  ...
  void rightRotate(Node *node) {
    ...
    // The left child is promoted to the current node's position, and the parent relationship is handled
  ...
  // Left rotation
  // This is the symmetrical case of right rotation, the logic is the same as right rotation
    ...
    while (target->parent && target->parent->color == Color::RED) {
      ...
      if (target->parent == target->parent->parent->left) {
        ...
        if (uncle && uncle->color == Color::RED) {
          target->parent->color = Color::BLACK; // Set the parent node to black
          uncle->color = Color::BLACK;          // Set the uncle node to black
          target->parent->parent->color = Color::RED; // Set the grandparent node to red
          ...
          // Adjust the colors of the parent node and grandparent node, and perform a right rotation
          target->parent->color = Color::BLACK;
          target->parent->parent->color = Color::RED;
          rightRotate(target->parent->parent);
        }
      } else {
        // When the parent node of the target node is the right child of the grandparent node, it is symmetrical to the above
        ...
        if (uncle && uncle->color == Color::RED) {
          ...
          // Adjust the colors of the parent node and grandparent node, and perform a left rotation
          target->parent->color = Color::BLACK;
          target->parent->parent->color = Color::RED;
  ...
  void insertNode(const Key &key, const Value &value) {
    // Create a new node, the node's color is initialized to red
    ...
    // After inserting a new node, call the insertFixup function to fix the red-black tree properties that may have been broken
    insertFixup(newNode);
  }
  ...
  void removeFixup(Node *node) {
    // If the node is Nil and has no parent node, it means it is the only node, and it returns directly
    ...
    while (node != root) {
      ...
      if (node == node->parent->left) {
        ...
        // Case 2: Both child nodes of the sibling node are black
        if (getColor(sibling->left) == Color::BLACK &&
          ...
          // Case 3: The right child node of the sibling node is black (the left child node is red)
          if (getColor(sibling->right) == Color::BLACK) {
            // Recolor the sibling node and the left child node of the sibling node, and perform a right rotation
          ...
          // Case 4: The right child node of the sibling node is red
        ...
        // When the node is the right child of its parent node, it is a symmetrical case
  ...
  void deleteNode(Node *del) {
    ...
    // If the node to be deleted has two children
      ...
      if (rep != del->right) {
        ...
        parentRP->left =
            child; // The left child of the parent node of the replacement node points to the child of the replacement node (because the replacement node is the minimum node, so it is impossible to have a left child)
        if (child != nullptr) {
    ...
    if (origCol == Color::BLACK) {
      // If a child node exists, perform a fixup operation
      if (child != nullptr) {
        removeFixup(child);
      }
      // If no child node exists, set the parent node of the Nil node (representing the empty node) to the parent node of the replacement node
      else {
        Nil->parent = parentRP;
...
public:
  using ValueType = std::list<Value>; // Change the type of Value to list
  ...
  void insert(const Key &key, const Value &value) {
    ...
    if (existingValues) {
      ...
      // If the key does not exist, create a new list and add the value, then insert it into the red-black tree
      ValueType values;
  ...
  void remove(const Key &key) {
    // Delete the entire key, including all its values
    ValueType *existingValues = rbTree.at(key);
    if (existingValues) {
  ...
  void remove(const Key &key, const Value &value) {
    ...
    if (existingValues) {
      ...
      size--;
      // If the list is empty after deleting the value, delete the key as well
...
private:
  RedBlackTree<Key, ValueType> rbTree; // Use ValueType as the value type of the red-black tree
  size_t size;
};

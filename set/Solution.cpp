...
private:
  ...
  void insertNode(const Key &key, const Value &value) {
    ...
    // Traverse the tree to find the correct position of the new node
    while (cmpNode) {
  ...
  void deleteNode(Node *del) {
    ...
    // If the original color is black, additional fixup operations are needed because deleting a black node may break the properties of the red-black tree
    if (origCol == Color::BLACK) {
      // If there is a child node, perform fixup operation
      if (child != nullptr) {
        removeFixup(child);
      }
      // If there is no child node, set the parent node of the Nil node (representing the empty node) to the parent node of the replacement node
      else {
        Nil->parent = parentRP;
        ...
        removeFixup(Nil);
        // Disconnect the Nil node from the tree, because in the red-black tree, the Nil node is usually separate
        dieConnectNil();
      }
    }

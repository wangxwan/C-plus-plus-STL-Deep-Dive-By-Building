...
private:
  ...
  void removeFixup(Node *node) {
    // If the node is Nil and has no parent, it means it is the only node, return directly
    ...
    while (node != root) {
      ...
      if (node == node->parent->left) {
        ...
        // When the node is the right child of its parent node, the symmetrical case
...
public:
  ...
  Value &operator[](const Key &key) {
    ...
    if (foundVal) {
      ...
      // If the key does not exist, insert a new key-value pair and return a reference to the newly inserted value
      Value defaultValue;
